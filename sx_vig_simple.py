#!/usr/bin/env python3
"""
SX Bet Vig Tracker - Simple Version
Minimal dependencies version for quick vig checking
"""
...
class SXBetSimpleVigTracker:
        ...
        try:
            ...
            if sport_id or league_id:
                ...
                for market in markets:
                    market_sport = market.get('sportId')
                    market_league = market.get('leagueId')
    ...
    def get_best_odds(self, market_hash: str) -> Optional[Dict]:
        ...
        try:
            # Use the working orders endpoint to get order book data
            response = self.session.get(f"{self.base_url}/orders?market={market_hash}")
            if response.status_code == 200:
                data = response.json()
                orders = data.get('data', [])
                if orders:
                    # Calculate best odds from orders
                    return self.calculate_best_odds_from_orders(orders)
            
            print(f"No orders available for market {market_hash[:10]}...")
            return None
            
        except requests.RequestException as e:
            print(f"Error fetching orders for market {market_hash[:10]}...: {e}")
            return None
    
    def calculate_best_odds_from_orders(self, orders: List[Dict]) -> Optional[Dict]:
        """Calculate best odds from order book data"""
        if not orders:
            return None
        
        # USDC base token address for mainnet
        USDC_BASE_TOKEN = "0x6629Ce1Cf35Cc1329ebB4F63202F3f197b3F050B"
        
        # Filter orders by USDC base token
    ...
    def print_market_table(self, vig_data: List[Dict], sort_by: str = 'vig'):
        ...
        print("="*100)
        print(f"{'Market':<30} {'Fixture':<25} {'Outcome 1':<10} {'Outcome 2':<10} {'Vig (%)':<8} {'Total Prob (%)':<12} {'Live':<5}")
        print("-" * 100)
        
        for data in vig_data:
            market_name = data['market_name'][:29]
            fixture = data['fixture'][:24]
            outcome1 = f"{data['outcome_one_odds']:.2f}"
            outcome2 = f"{data['outcome_two_odds']:.2f}"
            vig = f"{data['vig']:.2f}%"
            total_prob = f"{data['total_implied_prob']:.1f}%"
            live = "🟢" if data['is_live'] else "⚪"
            
            print(f"{market_name:<30} {fixture:<25} {outcome1:<10} {outcome2:<10} {vig:<8} {total_prob:<12} {live:<5}")
...
def main():
    parser = argparse.ArgumentParser(description='SX Bet Vig Tracker - Simple Version')
    ...
    # Initialize tracker
    tracker = SXBetSimpleVigTracker(use_testnet=args.testnet)
    
    print(f"SX Bet Vig Tracker - Simple Version")
